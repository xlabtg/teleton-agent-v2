...
export function Hooks() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [blockRes, trigRes, rulesRes] = await Promise.all([
        api.getBlocklist(),
        api.getTriggers(),
        api.getRules(),
      ]);
  ...
  // ── Structured rules actions ───────────────────────────────────────

  const handleCreateRule = async (data: Omit<StructuredRule, 'id' | 'order'>) => {
    setSaving(true);
    try {
  ...
  const handleUpdateRule = async (rule: StructuredRule) => {
    try {
      const res = await api.updateRule(rule.id, rule);
  ...
  const handleReorderRules = async (ids: string[]) => {
    ...
    setRules((prev) => {
      ...
      return ids
        ...
        .filter((r): r is StructuredRule => r !== null);
    });
    try {
  ...
  return (
    <div>
      ...
      {/* ── Mode switcher ── */}
      <div style={{ display: 'flex', gap: '2px', marginBottom: '16px', background: 'var(--surface)', borderRadius: '10px', padding: '3px', width: 'fit-content' }}>
        <button
          ...
          style={{
            ...
            fontSize: '13px',
            fontWeight: 500,
            background: mode === 'basic' ? 'var(--surface-active)' : 'transparent',
            color: mode === 'basic' ? 'var(--text)' : 'var(--text-secondary)',
        ...
        <button
          ...
          style={{
            ...
            fontSize: '13px',
            fontWeight: 500,
            background: mode === 'advanced' ? 'var(--surface-active)' : 'transparent',
            color: mode === 'advanced' ? 'var(--text)' : 'var(--text-secondary)',
            transition: 'background 0.15s, color 0.15s',
          }}
        >
          Visual Rule Builder
